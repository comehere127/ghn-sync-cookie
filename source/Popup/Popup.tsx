...
import type { Domain } from "../Background/storage";
...
const ErrorAlert = React.memo(({ error, domainSelected }: ErrorProps) => {
	const isExternal = React.useMemo(
		() => error.includes("No cookies found in cache"),
		[error],
	);
...
function Popup() {
	...
	const [{ error, isValidating }, dispatch] = React.useReducer(
		validationReducer,
		INITIAL_STATE,
	);
		...
		const validateTab = async () => {
			...
			try {
				...
				if (currentTab.environment === null) {
					dispatch({
						type: "SET_ERROR",
						payload: "Please using GHN Portal to sync cookies",
					});
					return;
				}
	...
	return (
		<section id="popup">
			...
			/>
			{syncError && (
				<ErrorAlert error={syncError.message} domainSelected={domainSelected} />
			)}
