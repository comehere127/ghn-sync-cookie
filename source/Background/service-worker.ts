import browser, { type Cookies, type Tabs } from "webextension-polyfill";
...
import { analyzeDomain, extractDomain } from "./domains";
...
async function setCookiesForLocalhost() {
	...
	const domainCookies = cookieList.filter(
		(cookie) => cookie.domain === domainSelected.domain,
	);

	if (domainCookies.length === 0) {
		throw new Error(
			`No cookies found in cache. Visit <b>${domainSelected.name}</b> to authenticate`,
		);
	}
	...
	const results = Promise.allSettled(
		targetOrigins.flatMap(() =>
			domainCookies.map(({ cookie }) =>
				setTargetCookie("http://localhost", cookie),
			),
		),
	);
	return results;
...
async function onCookieChanged(
	...
	if (
		!analyzeDomain(cookie.domain).isHostDomain ||
		!isKnownCookie(cookie.name)
	) {
		return;
	}
	try {
